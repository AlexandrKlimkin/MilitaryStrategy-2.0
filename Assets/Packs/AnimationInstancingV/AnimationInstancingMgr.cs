...
namespace AnimationInstancing
{
    ...
    public class AnimationInstancingMgr : AnimationInstancingReferenceHolder<AnimationInstancingMgr>
    {
        ...
        private static readonly int _BoneTexture = Shader.PropertyToID("_boneTexture");
        private static readonly int _BoneTextureWidth = Shader.PropertyToID("_boneTextureWidth");
        private static readonly int _BoneTextureHeight = Shader.PropertyToID("_boneTextureHeight");
        private static readonly int _BoneTextureBlockWidth = Shader.PropertyToID("_boneTextureBlockWidth");
        private static readonly int _BoneTextureBlockHeight = Shader.PropertyToID("_boneTextureBlockHeight");
        private static readonly int _FrameIndex = Shader.PropertyToID("frameIndex");
        private static readonly int _PREFrameIndex = Shader.PropertyToID("preFrameIndex");
        private static readonly int _TransitionProgress = Shader.PropertyToID("transitionProgress");

        public static AnimationInstancingMgr GetInstance()
        {
            return AnimationInstancingReferenceHolder<AnimationInstancingMgr>.Instance;
        }
                    ...
                    for (int k = 0; k != packageList.Length; ++k)
                    {
                        for (int i = 0; i != packageList[k].Count; ++i)
                        {
                            ...
                            for (int j = 0; j != package.subMeshCount; ++j)
                            {
...
#endif
                                    package.propertyBlock.SetFloatArray(_FrameIndex, data.frameIndex[k][i]);
                                    package.propertyBlock.SetFloatArray(_PREFrameIndex, data.preFrameIndex[k][i]);
                                    package.propertyBlock.SetFloatArray(_TransitionProgress, data.transitionProgress[k][i]);
                                ...
                                {
                                    package.material[j].SetFloat(_FrameIndex, data.frameIndex[k][i][0]);
                                    package.material[j].SetFloat(_PREFrameIndex, data.preFrameIndex[k][i][0]);
                                    package.material[j].SetFloat(_TransitionProgress, data.transitionProgress[k][i][0]);
